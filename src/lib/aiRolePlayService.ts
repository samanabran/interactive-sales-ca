...
// ============================================
// Company-specific persona mapping
// ============================================

export type B2BPersonaType = 
  // EIGER MARVEL HR personas
  | 'hr-manager'          // Operations Manager (Ali Asghar)
  | 'hr-business-owner'   // Future Partner (Hania Khan)
  | 'hr-finance-decider' // Finance Manager (Katherine)
  
  // SGC TECH AI personas
  | 'tech-it-manager'      // IT Manager (Technical Director)
  | 'tech-business-owner' // CEO/Founder
  | 'tech-operations';    // Head of Operations
...
export async function generateProspectResponse(
  ...
  updateConversationMemory(context.company, context.personaType, userMessage);
  const memoryPrompt = generateMemoryPrompt();
  
  // Generate response based on persona
  const response = generateB2BResponse(persona, context, userMessage, memoryPrompt);
  
  // Add natural speech patterns
  const withSpeechPatterns = addNaturalSpeechPatterns(
    response,
    persona.personality.talkative,
    getProcessingIntensity(context.messages.length)
  );
  
  return withSpeechPatterns;
}
...
function generateB2BResponse(
  ...
  if (message.includes('cost') || message.includes('price') || message.includes('budget')) {
    if (persona.objectionLikelihood.cost > 0.6) {
      return generateCostObjection(persona, companyName);
    }
  }
  ...
  if (message.includes('time') || message.includes('long') || message.includes('deploy')) {
    if (persona.objectionLikelihood.timeline > 0.5) {
      return generateTimelineObjection(persona, companyName);
    }
  }
  ...
  if (message.includes('quality') || message.includes('proof') || message.includes('reference')) {
    if (persona.objectionLikelihood.quality > 0.5) {
      return generateQualityObjection(persona, companyName);
    }
  }
  ...
  if (message.includes('competitor') || message.includes('sap') || message.includes('oracle')) {
    if (persona.objectionLikelihood.competition > 0.6) {
      return generateCompetitionObjection(persona, companyName);
    }
  }
  
  // ===== POSITIVE RESPONSES =====
  if (message.includes('roi') || message.includes('save') || message.includes('benefit')) {
    return generatePositiveResponse(persona, companyName);
  }
  
  // ===== DEFAULT PERSONA RESPONSE =====
  return generatePersonaDefaultResponse(persona, companyName, context.currentGoal);
}
...
function generateCostObjection(persona: B2BPersona, company: string): string {
...
function generateTimelineObjection(persona: B2BPersona, company: string): string {
  const responses: Record<string, string[]> = {
    'eiger-marvel-hr': [
...
function generateQualityObjection(persona: B2BPersona, company: string): string {
  const responses: Record<string, string[]> = {
    'eiger-marvel-hr': [
...
function generateCompetitionObjection(persona: B2BPersona, company: string): string {
  const responses: Record<string, string[]> = {
    'eiger-marvel-hr': [
...
function generatePositiveResponse(persona: B2BPersona, company: string): string {
  const responses: Record<string, string[]> = {
    'eiger-marvel-hr': [
...
function generatePersonaDefaultResponse(
  ...
  const options = responses[persona.type] || responses['hr-manager'];
  return options[Math.floor(Math.random() * options.length)];
}
