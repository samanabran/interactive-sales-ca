// EIGER MARVEL HR — Specialized Roleplay Component
// Blue-themed HR recruitment sales training
...
import {
  EIGER_MARVEL_CALL_FLOW,
  getEigerMarvelStageLabel,
  getEigerMarvelStageProgress,
  type EigerMarvelCallStage,
} from '@/lib/callScripts/eigerMarvelCallFlow';
import { detectObjectionType, getBestResponse } from '@/lib/objections/eigerMarvelObjections';

const EIGER_STAGES = EIGER_MARVEL_CALL_FLOW.stages;

function getEigerMetrics(messages: AIMessage[], objectionsHandled: string[]) {
  const hasRoi = messages.some(m => m.role === 'user' && m.content.toLowerCase().includes('roi'));
  const hasTimeline = messages.some(m => m.role === 'user' && m.content.toLowerCase().includes('time-to-hire'));
  const adherence = 60 + (hasRoi ? 15 : 0) + (hasTimeline ? 10 : 0) + Math.random() * 15;
  return {
    overallScore: adherence * 0.85 + (objectionsHandled.length > 0 ? 10 : 0),
    scriptAdherence: adherence,
    strengths: [
      ...(hasRoi ? ['Referenced ROI and cost savings — great anchoring'] : []),
      ...(hasTimeline ? ['Addressed time-to-hire KPI — shows product knowledge'] : []),
      objectionsHandled.length > 0 ? `Handled ${objectionsHandled.length} objection(s) professionally` : 'Maintained positive tone throughout',
    ],
    improvements: [
      !hasRoi ? 'Lead with the ROI calculator earlier — CFOs want numbers first' : 'Quantify the bad-hire cost angle more',
      'Offer a UAE reference client at the qualification stage',
      'Propose the 30-day pilot earlier to reduce commitment barrier',
    ],
  };
}

export default function EigerMarvelRolePlay() {
  const session = useRolePlaySession({
    company: 'eiger-marvel-hr',
    detectObjection: detectObjectionType,
    getCompanyMetrics: getEigerMetrics,
  });
  ...
  // (EigerMarvelRolePlay is only rendered when company === 'eiger-marvel-hr' by parent)
      ...
      <div className="min-h-screen bg-blue-50 p-4 md:p-6">
        <div className="max-w-4xl mx-auto space-y-6">
          <Card className="shadow-xl border-2 border-blue-200">
            <CardHeader className="text-center space-y-2 pb-6 bg-blue-600 text-white rounded-t-lg">
              ...
              <CardDescription className="text-blue-100 font-semibold">
                EIGER MARVEL HR practice with <span className="font-bold text-white">{session.selectedPersona?.name}</span>
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-6 pt-6">
              <div className="text-center p-8 bg-blue-50 rounded-2xl border-2 border-blue-200">
                ...
                <p className="text-base text-gray-600 font-semibold">
                  {session.sessionMetrics.overallScore >= 80 ? '🌟 Excellent recruiter pitch!' :
                   session.sessionMetrics.overallScore >= 60 ? '👍 Good job, keep refining!' :
                   '💪 Keep practicing the ROI approach!'}
                </p>
              </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {[
                  { label: '📋 HR Pitch Quality', value: session.sessionMetrics.scriptAdherence },
                  { label: '🛡️ Objection Handling', value: session.sessionMetrics.objectionHandling },
                  { label: '🤝 Rapport', value: session.sessionMetrics.rapport },
                  { label: '🎯 Closing', value: session.sessionMetrics.closing },
                ].map(({ label, value }) => (
                  <div key={label} className="p-4 bg-white rounded-xl border-2 border-blue-100 shadow-sm text-center">
              ...
              {session.sessionMetrics.objectionsEncountered.length > 0 && (
                <div className="p-4 bg-orange-50 rounded-xl border border-orange-200">
                  <p className="text-sm font-bold text-orange-800 mb-2">Objections Encountered:</p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="p-4 bg-green-50 rounded-xl border border-green-200">
                  ...
                  <ul className="space-y-1">{session.sessionMetrics.strengths.map((s, i) => <li key={i} className="text-sm text-green-700">• {s}</li>)}</ul>
                </div>
                <div className="p-4 bg-blue-50 rounded-xl border border-blue-200">
                  <p className="text-sm font-bold text-blue-800 mb-2">📈 Improvements:</p>
                  <ul className="space-y-1">{session.sessionMetrics.improvements.map((s, i) => <li key={i} className="text-sm text-blue-700">• {s}</li>)}</ul>
                </div>
              </div>

              <div className="flex flex-col sm:flex-row gap-3">
                <Button onClick={session.resetSession} className="flex-1 h-12 bg-blue-600 hover:bg-blue-700">
                  Practice Again with {session.selectedPersona?.name}
                </Button>
                <Button variant="outline" onClick={session.resetSession} className="flex-1 h-12 border-blue-300">
  ...
  // ===== Setup View (Persona Selection) =====
  if (session.showSetup) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-blue-50 to-white p-4 md:p-6">
        <div className="max-w-7xl mx-auto space-y-6">
          {/* Header */}
          <div className="flex items-center gap-4 bg-blue-600 text-white p-4 rounded-xl shadow-md">
            <Button variant="ghost" onClick={session.resetSession} className="text-white hover:bg-white/20 shrink-0">
              <ArrowLeft className="h-4 w-4 mr-2" />
              Back
            </Button>
            <div>
              <h1 className="text-2xl font-bold">🇦🇪 EIGER MARVEL HR — Sales Training</h1>
              <p className="text-blue-100 text-sm mt-0.5">Practice HR recruitment pitches with AI-powered B2B decision-makers</p>
            </div>
          </div>
          ...
          <Card className="border-2 border-blue-200 shadow-sm">
            <CardHeader>
              <CardTitle className="text-base text-blue-800">📋 EIGER MARVEL HR Sales Stages</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="flex flex-wrap gap-2">
                {EIGER_STAGES.map((stage, idx) => (
                  <div key={stage} className="flex items-center gap-1">
                    <Badge className="bg-blue-100 text-blue-800 border border-blue-200 font-semibold">{idx + 1}. {getEigerMarvelStageLabel(stage as EigerMarvelCallStage)}</Badge>
                    {idx < EIGER_STAGES.length - 1 && <span className="text-blue-300">→</span>}
                  </div>
                ))}
              </div>
            </CardContent>
          ...
          <Card className="border-2 border-blue-200 shadow-sm">
            <CardHeader>
              <CardTitle className="text-lg text-blue-800">🎤 Voice Agent Settings</CardTitle>
            ...
            <CardContent className="space-y-4">
              <div className="flex items-center justify-between p-4 bg-blue-50 rounded-xl border-2 border-blue-200">
                <div className="flex items-center gap-3">
                  {session.ttsEnabled ? <SpeakerHigh className="h-6 w-6 text-blue-600" weight="fill" /> : <SpeakerSlash className="h-6 w-6 text-gray-400" />}
                  <div>
                    <p className="text-sm font-semibold">Realistic HR Prospect Voices</p>
                ...
                <div className="grid grid-cols-3 gap-2">
                  {(['gemini', 'edge', 'deepgram'] as const).map(p => (
                    <Button key={p} variant={session.ttsProvider === p ? 'default' : 'outline'}
                      ...
                      className={`flex-col h-auto py-3 gap-1 ${session.ttsProvider === p ? 'bg-blue-600 hover:bg-blue-700' : ''}`}
                      size="sm"
                    >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {session.personas.map((persona: B2BPersona) => {
              ...
              return (
                <Card key={persona.id} className="cursor-pointer hover:shadow-xl hover:scale-[1.02] transition-all duration-200 border-2 hover:border-blue-500"
                  ...
                  <CardContent className="space-y-3">
                    ...
                    <div className="bg-blue-50 p-3 rounded-lg border border-blue-100">
                      <p className="text-xs font-bold mb-1.5 text-blue-700">Goals:</p>
                      <div className="flex flex-wrap gap-1">
                        {persona.goals.slice(0, 2).map((g, i) => (
                          <Badge key={i} className="text-xs bg-blue-600 text-white">{g}</Badge>
                        ))}
                      </div>
                    </div>
  ...
  return (
    <div className="h-screen flex flex-col bg-white">
      {/* Header */}
      <div className="border-b-2 border-blue-200 bg-blue-600 text-white shadow-sm p-3">
        <div className="max-w-7xl mx-auto flex items-center justify-between gap-3">
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-blue-100 font-semibold">🇦🇪 EIGER MARVEL HR · Stage: {getEigerMarvelStageLabel(session.currentStage as EigerMarvelCallStage)}</p>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            {/* Stage Progress */}
            <div className="hidden md:flex items-center gap-1">
              {EIGER_STAGES.map((s, i) => (
                <div key={s} className={`h-2 w-6 rounded-full transition-colors ${session.stagesReached.includes(s) ? 'bg-white' : i === EIGER_STAGES.indexOf(session.currentStage as EigerMarvelCallStage) ? 'bg-blue-200 animate-pulse' : 'bg-blue-400/40'}`} />
              ))}
            </div>
        ...
        <div className="max-w-7xl mx-auto mt-2">
          <Progress value={getEigerMarvelStageProgress(session.currentStage as EigerMarvelCallStage)} className="h-1.5 bg-blue-400/40" />
        </div>
      </div>
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 flex flex-col">
              ...
              <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                <div className={`max-w-[80%] rounded-2xl p-3 shadow-sm ${
                  msg.role === 'user'
                    ...
                    : 'bg-white border-2 border-blue-100 rounded-bl-md text-gray-800'
                }`}>
                ...
                <div className="bg-blue-50 border-2 border-blue-100 rounded-2xl p-3">
                  <div className="flex items-center gap-2">
                    <div className="flex space-x-1">
                      {[0, 0.1, 0.2].map((d, i) => <div key={i} className="w-2 h-2 bg-blue-400 rounded-full animate-bounce" style={{ animationDelay: `${d}s` }} />)}
                    </div>
          ...
          {/* Objection Coach Hint */}
          {objCoach && (
            <div className="mx-3 mb-2 p-3 bg-amber-50 border border-amber-200 rounded-xl">
              <div className="flex items-start gap-2">
                <Info className="h-4 w-4 text-amber-600 mt-0.5 shrink-0" />
                <div>
                  <p className="text-xs font-bold text-amber-800">Objection Coach — {objCoach.approach}:</p>
          ...
          <div className="border-t bg-white shadow-lg p-3">
            <div className="max-w-4xl mx-auto flex gap-2">
              ...
              <input type="text" value={session.currentMessage}
                ...
                placeholder="Type your HR sales pitch response..."
                className="flex-1 px-4 py-2 border-2 border-blue-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent text-sm"
                disabled={session.isProcessing}
              />
              <Button onClick={session.sendMessage} disabled={!session.currentMessage.trim() || session.isProcessing}
                className="shrink-0 h-10 px-4 bg-blue-600 hover:bg-blue-700">
